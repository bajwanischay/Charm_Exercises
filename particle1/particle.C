   ...
   public:
      Grid(int n,int k,float dim,int maxiter,CProxy_Main main){
        ...
        if(thisIndex.x<=thisIndex.y){
          for(int i=0;i<n;i++){
            Particle p=Particle(2,basex+((float)rand()*dim/(float)(RAND_MAX)),basey+((float)rand()*dim/(float)(RAND_MAX)));
            particle.push_back(p);
          }
        }
        //Red
        if(thisIndex.x>=k/4 && thisIndex.x<3*k/4 && thisIndex.y>=k/4 && thisIndex.y<3*k/4){
          for(int i=0;i<n;i++){
            Particle p=Particle(1,basex+((float)rand()*dim/(float)(RAND_MAX)),basey+((float)rand()*dim/(float)(RAND_MAX)));
            particle.push_back(p);
          }          
        }
        //Green
        if(thisIndex.x>=thisIndex.y){
          for(int i=0;i<n;i++){
            Particle p=Particle(4,basex+((float)rand()*dim/(float)(RAND_MAX)),basey+((float)rand()*dim/(float)(RAND_MAX)));
            particle.push_back(p);
          }
        }
        maxcount=4;
